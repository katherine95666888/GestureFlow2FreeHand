...
class Gesture:
    def __init__(self, hand="right"):
        self.hand = hand
        self.extension_threshold = 0.05  # Adjustable threshold for finger extension
        self.gestures = {
            "NUMBER_6": self.detect_number_6,  # Prioritize to avoid overlap with NUMBER_1
    ...
    def _is_thumb_extended(self, thumb_tip, thumb_ip, wrist):
        if self.hand == "right":
            return thumb_tip.x < thumb_ip.x  # Thumb extends left for right hand
        else:
            return thumb_tip.x > thumb_ip.x  # Thumb extends right for left hand

    def _is_finger_extended(self, tip, pip, mcp):
        tip_to_mcp = ((tip.x - mcp.x) ** 2 + (tip.y - mcp.y) ** 2) ** 0.5
        pip_to_mcp = ((pip.x - mcp.x) ** 2 + (pip.y - mcp.y) ** 2) ** 0.5
        return tip_to_mcp > pip_to_mcp + self.extension_threshold

    def detect_number_1(self, landmarks):
        thumb_folded = not self._is_thumb_extended(landmarks[4], landmarks[3], landmarks[0])
        index_extended = self._is_finger_extended(landmarks[8], landmarks[6], landmarks[5])
        middle_folded = not self._is_finger_extended(landmarks[12], landmarks[10], landmarks[9])
        ring_folded = not self._is_finger_extended(landmarks[16], landmarks[14], landmarks[13])
        pinky_folded = not self._is_finger_extended(landmarks[20], landmarks[18], landmarks[17])
        return thumb_folded and index_extended and middle_folded and ring_folded and pinky_folded

    def detect_number_2(self, landmarks):
        thumb_folded = not self._is_thumb_extended(landmarks[4], landmarks[3], landmarks[0])
        index_extended = self._is_finger_extended(landmarks[8], landmarks[6], landmarks[5])
        middle_extended = self._is_finger_extended(landmarks[12], landmarks[10], landmarks[9])
        ring_folded = not self._is_finger_extended(landmarks[16], landmarks[14], landmarks[13])
        pinky_folded = not self._is_finger_extended(landmarks[20], landmarks[18], landmarks[17])
        return thumb_folded and index_extended and middle_extended and ring_folded and pinky_folded

    def detect_number_3(self, landmarks):
        thumb_folded = not self._is_thumb_extended(landmarks[4], landmarks[3], landmarks[0])
        index_extended = self._is_finger_extended(landmarks[8], landmarks[6], landmarks[5])
        middle_extended = self._is_finger_extended(landmarks[12], landmarks[10], landmarks[9])
        ring_extended = self._is_finger_extended(landmarks[16], landmarks[14], landmarks[13])
        pinky_folded = not self._is_finger_extended(landmarks[20], landmarks[18], landmarks[17])
    ...
    def detect_number_4(self, landmarks):
        ...
        middle_extended = self._is_finger_extended(landmarks[12], landmarks[10], landmarks[9])
        ring_extended = self._is_finger_extended(landmarks[16], landmarks[14], landmarks[13])
        pinky_extended = self._is_finger_extended(landmarks[20], landmarks[18], landmarks[17])
        return thumb_folded and index_extended and middle_extended and ring_extended and pinky_extended
    ...
    def detect_number_6(self, landmarks):
        ...
        ring_folded = not self._is_finger_extended(landmarks[16], landmarks[14], landmarks[13])
        pinky_folded = not self._is_finger_extended(landmarks[20], landmarks[18], landmarks[17])
        print(f"NUMBER_6 check: Thumb extended={thumb_extended}, Index extended={index_extended}, Middle folded={middle_folded}, Ring folded={ring_folded}, Pinky folded={pinky_folded}")
    ...
    def detect_fist(self, landmarks):
        thumb_folded = not self._is_thumb_extended(landmarks[4], landmarks[3], landmarks[0])
        index_folded = not self._is_finger_extended(landmarks[8], landmarks[6], landmarks[5])
        middle_folded = not self._is_finger_extended(landmarks[12], landmarks[10], landmarks[9])
        ring_folded = not self._is_finger_extended(landmarks[16], landmarks[14], landmarks[13])
        pinky_folded = not self._is_finger_extended(landmarks[20], landmarks[18], landmarks[17])
...
class GestureApp:
    ...
    def set_hand(self, event):
        ...
        self.gesture.hand = self.hand  # Update gesture handedness
                ...
                if results.multi_hand_landmarks:
                    for hand_landmarks, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
                        if handedness.classification[0].label.lower() == self.hand:
                            ...
                            # Visual feedback: colored circles on finger tips (green=extended, red=folded)
                            thumb_color = (0, 255, 0) if self.gesture._is_thumb_extended(lm[4], lm[3], lm[0]) else (0, 0, 255)
                            cv2.circle(frame, (int(lm[4].x * CAM_WIDTH), int(lm[4].y * CAM_HEIGHT)), 10, thumb_color, -1)

                            index_color = (0, 255, 0) if self.gesture._is_finger_extended(lm[8], lm[6], lm[5]) else (0, 0, 255)
                            cv2.circle(frame, (int(lm[8].x * CAM_WIDTH), int(lm[8].y * CAM_HEIGHT)), 10, index_color, -1)

                            middle_color = (0, 255, 0) if self.gesture._is_finger_extended(lm[12], lm[10], lm[9]) else (0, 0, 255)
                            cv2.circle(frame, (int(lm[12].x * CAM_WIDTH), int(lm[12].y * CAM_HEIGHT)), 10, middle_color, -1)

                            ring_color = (0, 255, 0) if self.gesture._is_finger_extended(lm[16], lm[14], lm[13]) else (0, 0, 255)
                            cv2.circle(frame, (int(lm[16].x * CAM_WIDTH), int(lm[16].y * CAM_HEIGHT)), 10, ring_color, -1)

                            pinky_color = (0, 255, 0) if self.gesture._is_finger_extended(lm[20], lm[18], lm[17]) else (0, 0, 255)
                            cv2.circle(frame, (int(lm[20].x * CAM_WIDTH), int(lm[20].y * CAM_HEIGHT)), 10, pinky_color, -1)
