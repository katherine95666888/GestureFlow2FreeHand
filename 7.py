...
class Gesture:
    def __init__(self, hand="right"):
        self.hand = hand  # Store hand preference
        self.gestures = {
    ...
    def _is_finger_extended(self, tip, pip, mcp, threshold=0.05):
        """Check if a finger is extended by comparing the distance from tip to MCP vs. PIP to MCP."""
    ...
    def _is_thumb_extended(self, thumb_tip, thumb_ip, wrist):
        """Check if thumb is extended, adjusting for handedness."""
        if self.hand == "right":
            return thumb_tip.x < thumb_ip.x  # Right hand: thumb tip to the left of IP
        else:
            return thumb_tip.x > thumb_ip.x  # Left hand: thumb tip to the right of IP

    def detect_number_1(self, landmarks):
        return (self._is_finger_extended(landmarks[8], landmarks[6], landmarks[5]) and  # Index extended
                all(not self._is_finger_extended(landmarks[i], landmarks[i-2], landmarks[i-3]) for i in [12, 16, 20]))  # Others folded

    def detect_number_2(self, landmarks):
        return (self._is_finger_extended(landmarks[8], landmarks[6], landmarks[5]) and  # Index extended
                self._is_finger_extended(landmarks[12], landmarks[10], landmarks[9]) and  # Middle extended
                all(not self._is_finger_extended(landmarks[i], landmarks[i-2], landmarks[i-3]) for i in [16, 20]))  # Others folded

    def detect_number_3(self, landmarks):
        return (self._is_finger_extended(landmarks[8], landmarks[6], landmarks[5]) and  # Index extended
                self._is_finger_extended(landmarks[12], landmarks[10], landmarks[9]) and  # Middle extended
                self._is_finger_extended(landmarks[16], landmarks[14], landmarks[13]) and  # Ring extended
                not self._is_finger_extended(landmarks[20], landmarks[18], landmarks[17]))  # Pinky folded

    def detect_number_4(self, landmarks):
        return (all(self._is_finger_extended(landmarks[i], landmarks[i-2], landmarks[i-3]) for i in [8, 12, 16, 20]) and  # All fingers extended
                not self._is_thumb_extended(landmarks[4], landmarks[3], landmarks[0]))  # Thumb folded

    def detect_number_5(self, landmarks):
        return (all(self._is_finger_extended(landmarks[i], landmarks[i-2], landmarks[i-3]) for i in [8, 12, 16, 20]) and  # All fingers extended
                self._is_thumb_extended(landmarks[4], landmarks[3], landmarks[0]))  # Thumb extended
    ...
    def detect_number_6(self, landmarks):
        ...
        others_folded = [not self._is_finger_extended(landmarks[i], landmarks[i-2], landmarks[i-3]) for i in [12, 16, 20]]
        # Debug output
        print(f"NUMBER_6 check: Thumb extended={thumb_extended}, Index extended={index_extended}, Others folded={others_folded}")
        if thumb_extended and index_extended and all(others_folded):
            return True
        else:
            print(f"Gesture detected: None (NUMBER_6 failed: {'thumb' if not thumb_extended else 'index' if not index_extended else 'others not folded'})")
            return False
    ...
    def detect_fist(self, landmarks):
        return all(not self._is_finger_extended(landmarks[i], landmarks[i-2], landmarks[i-3]) for i in [8, 12, 16, 20])
...
class GestureApp:
    ...
    def open_binding_settings(self):
        ...
        for action_name in self.available_actions:
            ...
            old_gesture = next((g for g, (f, an) in self.bindings.items() if an == action_name), "None")
            combo.set(old_gesture)
    ...
    def update_binding(self, action_name, gesture):
        ...
        old_gesture = next((g for g, (f, an) in self.bindings.items() if an == action_name), None)
        if old_gesture:
            del self.bindings[old_gesture]
        
        if gesture == "None":
            pass  # No new binding
        else:
            # Remove if this gesture was bound elsewhere
                ...
                if results.multi_hand_landmarks:
                    for hand_landmarks, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
                        if handedness.classification[0].label.lower() == self.hand:
                            ...
                            # Visual feedback for finger status (always show for debugging)
                            ...
                            # Draw circles: green for extended, red for folded
