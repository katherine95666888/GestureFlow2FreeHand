...
# Camera settings (default resolution, full frame as ROI)
...
class Gesture:
    ...
    #     distance = ((thumb_tip.x - middle_tip.x) ** 2 + (thumb_tip.y - middle_tip.y) ** 0.5
    #     return distance < 0.05
    def detect_orchid(self, landmarks):
        thumb_tip, middle_tip = landmarks[4], landmarks[12]
        index_tip, ring_tip = landmarks[8], landmarks[16]
        distance = ((thumb_tip.x - middle_tip.x) ** 2 + (thumb_tip.y - middle_tip.y) ** 2) ** 0.5
        # Ensure other fingers are not pinched
...
class GestureApp:
    ...
    def update_camera(self):
        if self.is_running and self.camera_on and self.cap:
            ret, frame = self.cap.read()
            if ret:
                frame = cv2.flip(frame, 1)
